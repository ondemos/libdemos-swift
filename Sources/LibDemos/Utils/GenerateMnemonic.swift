...
public func generateMnemonic(strength: Int = 128) throws -> [String] {
  ...
  guard wordlist.count > 0 else { throw MnemonicError.wordlistIsEmpty }
  ...
  let entropyBits = entropy.map { String($0, radix: 2).leftPadding(toLength: 8, withPad: "0") }
    .joined()

  let CS = strength / 32
  let entropyHash = try SHA512(data: Data(entropy))
  let checksumBits = entropyHash.map { String($0, radix: 2).leftPadding(toLength: 8, withPad: "0") }
    .joined().prefix(CS)
