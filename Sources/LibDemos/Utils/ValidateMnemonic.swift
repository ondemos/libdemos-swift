...
public func mnemonicToEntropy(words: [String]) throws -> Data {
  guard words.count % 3 == 0 else { throw MnemonicError.numberOfWordsInMnemonicMustBeMultipleOf3 }

  guard words.count >= 12 && words.count <= 48 else { throw MnemonicError.invalidMnemonicSize }

  let wordlist = Wordlist.data

  guard wordlist.count > 48 else { throw MnemonicError.wordlistIsEmpty }

  let bits =
    try words.map { word -> String in
  ...
  let entropy = try entropyBitsMatched.map { bin -> UInt8 in
    guard let byte = UInt8(bin, radix: 2) else { throw MnemonicError.invalidMnemonicSize }

    return byte
  }

  guard entropy.count % 4 == 0 else { throw MnemonicError.entropyLengthNotMultipleOf4 }
  ...
  let newChecksum =
    entropyHash.reduce("") { str, byte in
      str + String(byte, radix: 2).leftPadding(toLength: 8, withPad: "0")
    }
    .prefix(CS)

  if newChecksum != checksumBits {
    throw MnemonicError.invalidMnemonicChecksum(
      newChecksum: String(newChecksum),
